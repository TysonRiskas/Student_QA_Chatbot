"""
Student Q&A Chatbot - Flask Web Application (SQL Version)
A web-based chatbot using PostgreSQL/SQLite database
"""
...
class ChatbotManager:
    ...
    def save_conversation(self, question, answer, user_id=None, session_id=None, user_info=None):
        """Save a Q&A pair to database."""
        try:
...
def index():
    ...
    if 'user_info' in session and session.get('user_info', {}).get('is_registered'):
        user_id = session.get('user_id')
        if user_id:
            user = User.query.get(user_id)
            if not user:
                # User doesn't exist in SQL database, clear session
                session.clear()
...
def force_logout():
    """Force logout - clears session and shows message."""
    session.clear()
    return redirect(url_for('login'))
...
def ask():
    ...
    user_id = session.get('user_id') if session.get('user_info', {}).get('is_registered') else None
    chatbot.save_conversation(
        question, 
        answer, 
        user_id=user_id,
        session_id=session.get('session_id'),
        user_info=session.get('user_info') if not user_id else None
    )
...
def export_data():
    """Export all conversation data (admin/backend route - PROTECTED)."""
...
if __name__ == '__main__':
    ...
    print("Using SQL Database (PostgreSQL/SQLite)")
    print("=" * 60)
    
    # Get port from environment (Heroku sets PORT automatically)
